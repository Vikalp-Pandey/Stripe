...
export const onboardSeller = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    return sendResponse(res, 200, 'Seller Account', account);
    // return sendRedirect(res,onboardingUrl)
  }
);

export const onboardingSuccessRedirect = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    // );
    return sendRedirect(res, `http:localhost:5173/payment`);
  }
);

export const refreshOnBoarding = asyncHandler(
  async (req: Request, res: Response) => {
    const { accountId } = req.params;
    if (typeof accountId == 'string') {
      const link = await stripeService.createAccountLink(accountId);
      return sendResponse(res, 200, 'Account Refreshed Successfully', {
        onBoarding: link,
      });
    }
...
export const createPaymentTransfer = asyncHandler(
  async (req: Request, res: Response) => {
